...
프로미스로 구현된 비동기 함수 내부에서 Promise 객체를 생성하고 반환한다.

비동기 처리는 Promise 생성자 함수가 인수로 전달받은 콜백 함수 내부에서 수행한다.

Promise 객체의 상태 정보는 resolve 또는 reject 함수를 호출하는것으로 결정

resolve 또는 reject 함수를 호출할 때 비동기 처리 결과 또는 에러는 Promise 객체의 후속 처리 메서드에게 전달한다.
...
프로미스로 구현된 비동기 함수를 호출하는 측에서는 Promise 객체의 후속처리 메서드 `then, catch, finally` 를 통해 비동기 처리 결과 또는 에러 메세지를 전달받아 후속 처리를 수행한다.

모든 후속처리 메서드 또한 비동기식으로 동작한다.

Promise 객체는 비동기 처리가 성공/실패 결과에 따른 상태정보에 따라
후속 처리 메서드를 체이닝 방식으로 호출한다.
...
- Promise 객체의 후속 처리 메서드 catch
  (내부적으로 then 메서드를 호출한다.) 

then 메서드의 두 번째 콜백 함수는 비동기 처리에서 발생한 에러만을 캐치한다. (then 메서드 내부의 에러를 캐치하지 못한다.)
catch 메서드는 비동기에서 발생한 에러 + then 메서드 내부에서 발생한 에러도 캐치한다.

=  에러 처리는 catch 메서드를 사용하는편이 보다 효율적!(가독성 면에서도)



## 프로미스 체이닝

비동기 함수의 처리결과를 가지고 다른 비동기 함수를 호출해야하는 경우 비동기 함수의 호출이 중첩이 되어 복잡도가 높아지는 콜백헬이 발생한다.

Promise 객체를 반환한 비동기 함수는 프로미스 후속처리메서드를 사용할 수 있다. 이 후속 처리 메서드는 모두 Promise 객체를 반환한다.

따라서 후속처리 메서드를 체이닝 하여 호출할 수 있다. 
이로써 콜백헬을 해결한다.
